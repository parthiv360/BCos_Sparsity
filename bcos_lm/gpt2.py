# This code is taken from the Bcos_lm repository(https://github.com/Ewanwong/bcos_lm)
    ...
    for name, array in zip(names, arrays):
        ...
        for m_name in name:
            ...
            if scope_names[0] in ["w", "g"]:
                ...
                pointer = getattr(pointer,"wpe").weight
            elif scope_names[0] == "wte":
                pointer = getattr(pointer,"wte").weight
            else:
                pointer = getattr(pointer,scope_names[0])
            
            if len(scope_names)>=2:
                pointer = pointer[int(scope_names[1])]
...
class GPT2Attention(nn.Module):
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        if not self.is_cross_attention:
            ...
            # Dimension is [batch_size, num_heads, seq_length, head_dim]
